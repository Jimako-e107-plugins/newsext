...
//{$sub_action} IN (ne.news_categories) 
				$query = "SELECT ne.news_id FROM #news_extended AS ne WHERE FIND_IN_SET({$sub_action} , ne.news_categories)  ";
	...
	private function getPaginationParms($total, $amount)
	{
		...
		$opts = [
			...
			'amount'      => (int) $amount,
			'current'     => $this->from,
			'url'         => e107::url("newsext/category-page", $this->newsUrlparms),
		];
